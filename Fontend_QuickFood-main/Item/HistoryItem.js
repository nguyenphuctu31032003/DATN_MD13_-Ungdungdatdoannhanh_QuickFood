...
const HistoryItem = ({item, navigation, isCancel, setIsCancel}) => {
            ...
            onPress: async () => {
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  ToastAndroid.show('Hủy đơn hàng thành công',
                      ToastAndroid.SHORT);
                ...
                console.error('Lỗi khi gọi API cập nhật trạng thái đơn hàng:',
                    error);
              }
  ...
  const handledatlai = async () => {
    ...
    navigation.navigate('PayScreen', {
      products: item.products, dataUid
    });
  }
  ...
  return (
      <View>
        ...
        <TouchableOpacity style={styles.itemContainer} onPress={handleDeital}>
          <Image source={require('./../Image/iconaddm.png')}
                 style={styles.image}/>
          <View style={styles.item}>
            <Text style={styles.name}>{productNames}</Text>
            <Text style={styles.detail}>Mã ĐH: {item._id}</Text>
            <Text style={styles.detail}>Thời gian: {formatTime(
                item.time)}</Text>
            <Text style={styles.detail}>Phương thức thanh
              toán: {item.phuongthucthanhtoan}</Text>
            <Text style={styles.detail}>Trạng thái: {getStatusText(
                item.status)}</Text>
            <View style={styles.buttonContainer}>
              ...
              {item.status === 1 || item.status === 2 && (
                  <TouchableOpacity style={styles.buttonLoad} onPress={() => {/* Thêm hành động khi nhấn nút */
                    ...
                    <Text style={styles.buttonText}>Vui lòng chờ món ngon đang
                      đến...</Text>
                  </TouchableOpacity>
              )}
              {item.status === 3 && (
