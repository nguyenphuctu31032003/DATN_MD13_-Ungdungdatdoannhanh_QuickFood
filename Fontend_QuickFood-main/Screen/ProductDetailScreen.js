...
const ProductDetailScreen = ({navigation, route}) => {
  ...
  const submitComment = async () => {
    ...
    if (!newComment || newComment.trim() === "") {
      ToastAndroid.show('Người dùng phải nhập bình luận, không được để trống!',
          ToastAndroid.SHORT);
      return;
    }
  ...
  const saveObjectToMongoDB = (object) => {
    ...
    .then((data) => {
      ...
      if (data.msg == 'đang thêm sản phẩm không cùng 1 cửa hàng vào giỏ hàng') {
        ...
        console.log(
            "sản phẩm thêm vào đang trùng id với sản phẩm có trong giỏ hàng");
        Alert.alert(
            'Cảnh báo!!',
  ...
  const getdataproductFavorite = async () => {
    ...
    .then(response => response.json())
    .then(data => {
      let filteredComments = data[0].listFavorite.filter(
          dataproducts => dataproducts._id === product._id);
        ...
        <View style={styles.container}>
          ...
          <ScrollView>
            ...
            <View
              ...
              <Text style={{padding: 5, fontSize: 20, fontWeight: 'bold'}}>Món
                ăn yêu thích </Text>
              {productfavorite ? (
                  <View>
                    <TouchableOpacity
                        onPress={() =>
                            Alert.alert("Xác nhận",
                                "Bạn có chắc chắn muốn hủy sản phẩm khỏi danh sách yêu thích không?",
                                [
                                  {
                                    text: "Hủy",
                                    style: "cancel",
