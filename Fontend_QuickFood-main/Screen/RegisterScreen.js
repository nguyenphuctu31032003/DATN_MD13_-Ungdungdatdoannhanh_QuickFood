...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    .then(res => {
      if (res.status === 200) {
        ...
        ToastAndroid.show('Tên tài khoản đã được sử dụng!! ',
            ToastAndroid.SHORT);
      } else if (res.status === 502) {
        ToastAndroid.show('Số điện thoại đã được sử dụng!! ',
            ToastAndroid.SHORT);
      } else {
  ...
  return (
      <View style={styles.container}>
        ...
        />
        {validateUser !== '' && <Text
            style={styles.errorText}>{validateUser}</Text>}
        <TextInput
            label="Số điện thoại"
            value={phone}
        ...
        />
        {validatePhone !== '' && <Text
            style={styles.errorText}>{validatePhone}</Text>}
        ...
        {validatePass !== '' && <Text
            style={styles.errorText}>{validatePass}</Text>}
        <View style={styles.passwordContainer}>
          <TextInput
        ...
        {validaRepass !== '' && <Text
            style={styles.errorText}>{validaRepass}</Text>}
        <Button
            mode="contained"
